...
## 7. 规则与验收标准（Spec-Driven 补充）

### 7.1 时间语义与时区
- 统一使用 **UTC** 存储，前端展示可按本地时区转换。
- 预约时间区间采用 **左闭右开**：[start_time, end_time)。
- 不允许创建 `end_time <= start_time` 的预约。

### 7.2 状态定义
- **Node.status**: `online` / `offline` / `maintenance`
- **Device.status**: `idle` / `reserved` / `offline`（由节点状态派生或被预约锁定）
- **Reservation.status**: `active` / `released` / `expired` / `canceled`

### 7.3 预约生命周期（核心规则）
1. **active**: 当前时间在预约时间段内且未被释放。
2. **released**: 用户/管理员主动释放。
3. **expired**: end_time 到期自动过期。
4. **canceled**: 预约尚未开始前被取消。

### 7.4 权限矩阵（关键操作）
- **Admin**: 管理用户白名单、节点/设备录入、强制释放、查看全量报表。
- **Developer**: 创建/取消/释放个人预约、管理个人 SSH 公钥、查看资源状态。
- **匿名用户**: 仅能访问登录页与公开信息（如有）。

### 7.5 数据校验规则
- **SSH 公钥**: 格式必须满足 `ssh-rsa/ssh-ed25519` 等合法公钥前缀，重复公钥拒绝入库。
- **节点名称**: 全局唯一；IP/Port 组合不得重复。
- **设备索引**: 同一节点下 `device_index` 必须唯一。
- **预约**: 设备模式下必须提供 `device_ids` 且非空；整机模式下必须为空。
...
### 8.3 风险
- 节点离线或 Agent 异常导致授权状态短暂不一致。
- SQLite 在高并发写入场景可能成为瓶颈（需迁移策略）。

---

## 9. 质量目标（SLO 草案）

- 预约冲突检测响应时间：P95 < 300ms。
- 授权下发延迟：P95 < 5s（由 Agent 轮询周期决定）。
- 监控数据刷新：<= 30s。
